...
Layout (``{lg}`` = ``wbb``/``mbb``, ``{season}`` = ending year, e.g. 2026)::
...
def rebuild_missing(
    kind: str, season: int, *, league: str = LEAGUE, root: Union[str, Path], overwrite: bool = False
) -> int:
...
def build_season_aggregate(kind: str, season: int, *, league: str = LEAGUE, root: Union[str, Path]) -> pl.DataFrame:
...
def build_teams(season: int, *, league: str = LEAGUE, root: Union[str, Path], overwrite: bool = True) -> pl.DataFrame:
...
def _main() -> None:
    ...
    parser.add_argument("--league", default=LEAGUE, help=f"League slug: 'wbb' or 'mbb' (default: {LEAGUE}).")
