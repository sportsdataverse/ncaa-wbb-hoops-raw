...
Runs the SAME small probe (default 10 known-good contests x 2 game-detail
...
# Known-good captured NCAA contests -- stats.ncaa.org contest ids are a single
# sport-agnostic namespace, so these men's games probe the WBB transport just as
# well -- any non-clean result is a vendor/challenge failure, not a bad id.
DEFAULT_CONTEST_IDS = [
    "6388769",
    "6388905",
    "6388907",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    header = (
        ...
        f"{len(contest_ids)} NCAA contests x {len(PAGES)} pages "
