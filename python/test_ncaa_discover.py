...
def test_discover_season_dedups_across_teams() -> None:
    ...
    two_teams = discover_season(
        _SEASON, team_ids=[_TEAM_ID, 700000], fetch_fn=lambda tid: _HTML
    )
...
def test_write_master_merges_and_preserves_captured() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        discover_season(
            _SEASON, team_ids=[_TEAM_ID], fetch_fn=lambda tid: _HTML, root=root
        )
...
def test_discover_season_present_season_selects_teams_from_real_crosswalk() -> None:
    ...
    df = discover_season(
        _SEASON, league="wbb", limit_teams=3, fetch_fn=lambda tid: _HTML
    )
    assert df.height > 0
...
def test_discover_season_league_wbb_selects_wbb_crosswalk_not_mbb() -> None:
    ...
    wbb_ids = set(
        ncaa_wbb_team_ids()
        .filter(pl.col("season") == season_str)
        .get_column("id")
        .to_list()
    )
    mbb_ids = set(
