...
# season parquet.
#
#   wbb/schedules/html/{season}/{team_id}.html   <- from run_01_schedules.sh
#   wbb/schedules/json/{season}/{team_id}.json   <- from run_01_schedules.sh
#   wbb/schedules/parquet/{season}.parquet       <- compiled HERE (one file)
#   wbb/rosters/html/{season}/{team_id}.html     <- from run_04_rosters.sh
#   wbb/rosters/json/{season}/{team_id}.json     <- from run_04_rosters.sh
...
"$PY" python/ncaa_wbb_05_datasets_build.py "$@" 2>&1 | tee -a "${LOG}"
rc=${PIPESTATUS[0]}
